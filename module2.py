...
def auxiliary_discriminator(image, options, reuse=False, name="auxiliary_discriminator"):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        network = self.add_layer(self.network, n_filter = base_filter, with_bn = False, name = '1')
        network = self.add_layer(self.network, n_filter = base_filter * (2 ** 1), name = '2')
        network = self.add_layer(self.network, n_filter = base_filter * (2 ** 2), name = '3')
        network = self.add_layer(self.network, n_filter = base_filter * (2 ** 3), name = '4')
        network = tl.layers.FlattenLayer(self.network)
        network = tl.layers.DenseLayer(self.network, n_units = 1)
        return self.network
